...
find "$OUTPUT"/FunSec_Output/SignalP -maxdepth 1 -type f -exec basename {} \; | while read -r i
do
	"$SCRIPT_DIR"/bin/tmhmm-2.0c/bin/tmhmm -short "$OUTPUT"/FunSec_Output/SignalP/"$i" | \
...
find "$OUTPUT"/FunSec_Output/TMHMM -maxdepth 1 -type f -exec basename {} \; | while read -r i 
do
...
find "$OUTPUT"/FunSec_Output/SignalP_TMHMM_Phobius -maxdepth 1 -type f -exec basename {} \; | while read -r i
do
	"$SCRIPT_DIR"/bin/WoLFPSort-master/bin/runWolfPsortSummary fungi < "$OUTPUT"/FunSec_Output/SignalP_TMHMM_Phobius/"$i" | \
...
find "$OUTPUT"/FunSec_Output/SignalP_TMHMM_Phobius -maxdepth 1 -type f -exec basename {} \; | while read -r i
do
	"$SCRIPT_DIR"/bin/lin/pc_fm "$OUTPUT"/FunSec_Output/SignalP_TMHMM_Phobius/"$i" -NODB -NOOL | \
...
find "$OUTPUT"/FunSec_Output/SignalP_TMHMM_Phobius -maxdepth 1 -type f -exec basename {} \; | while read -r i
do
	comm -12 "$OUTPUT"/FunSec_Output/WolfPsort/"$i" "$OUTPUT"/FunSec_Output/ProtComp/"$i" | \
	tee "$OUTPUT"/FunSec_Output/WolfPsort_ProtComp/Headers/"$i"
done
...
find "$OUTPUT"/FunSec_Output/WolfPsort_ProtComp -maxdepth 1 -type f -exec basename {} \; | while read -r i
do
	"$SCRIPT_DIR"/bin/ps_scan/ps_scan.pl -p "[KRHQSA]-[DENQ]-E-L>" "$OUTPUT"/FunSec_Output/WolfPsort_ProtComp/"$i" | \
...
then 
	...
	find "$OUTPUT"/FunSec_Output/Ps-scan -maxdepth 1 -type f -exec basename {} \; | while read -r i
	do
